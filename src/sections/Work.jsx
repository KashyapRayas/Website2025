...
const Work = forwardRef(({ handleProjectSelect }, ref) => {
  ...
  const ctxRef = useRef(null); // Keep reference to the scaled context
  ...
  const activeProject = useMemo(() => {
    if (activeIndex < 0 || activeIndex >= projectsData.projects.length) {
      return {};
    }
    return projectsData.projects[activeIndex] ?? {};
  }, [activeIndex]);
  ...
  // --- 1. RESIZE LOGIC (High Quality & Correct Dimensions) ---
  useEffect(() => {
    const handleResize = () => {
      const canvas = canvasRef.current;
      if (!canvas) return;

      // 1. Get the size of the parent container (the div wrapping the canvas)
      // This ensures the canvas never exceeds the parent layout.
      ...
      // Set the internal buffer size (Resolution)
      canvas.width = rect.width * dpr;
      canvas.height = rect.height * dpr;

      // 3. Scale the drawing context
      // This allows us to draw using CSS units, and the browser handles the pixel density.
      ...
      // Force a redraw of the current project if resizing happens
      if (activeProject) {
        // We trigger the draw logic manually here to prevent blank canvas on resize
        const src = activeProject.img ? BASE_PATH + activeProject.img : FALLBACK_IMG_SRC;
  ...
  // Helper to draw image "Cover" style (like CSS object-fit: cover)
  const draw = (ctx, img, width, height) => {
      if (!img || !img.naturalWidth) return;

      // Ensure opacity is reset (fixes the double exposure ghosting)
      ctx.globalAlpha = 1;
  ...
  // --- 2. DRAWING LOGIC (Instant Switch - No Animation) ---
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    ...
    // Get dimensions based on the CSS size (Logical Pixels)
    ...
    if (img.complete && img.naturalWidth > 0) {
        ...
        img.onload = () => {
            // Check if this image is still the active one before drawing
            // (Prevents race conditions on fast hover)
            const currentSrc = activeProject.img ? BASE_PATH + activeProject.img : FALLBACK_IMG_SRC;
  ...
  }, [activeProject]); // Only re-run when the project changes
  ...
  // Simple pop effect on canvas when changing projects
  useGSAP(() => {
